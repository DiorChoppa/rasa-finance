    ...
    async def run(
        ...
        if tracker.get_slot("zz_confirm_form") == "yes":
            ...
            to_account_number = profile_db.get_account_number(
                profile_db.get_account_from_session_id(
                    tracker.sender_id)
            )
            ...
            amount_converted= float(tracker.get_slot("amount_converted"))
            slots["amount_converted"] = amount_converted #+ amount_of_money
        else:
            dispatcher.utter_message(response="utter_converting_cancelled")
