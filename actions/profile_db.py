...
class CreditCard(Base):
    ...
    current_balance = Column(REAL) #dollar
    current_ruble_balance = Column(REAL) #ruble
...
class ProfileDB:
    ...
    def get_credit_card_balance_ruble(
        self,
        session_id: Text,
        credit_card_name: Text,
        balance_type: Text = "current_ruble_balance",
    ):
        """Get the balance for a credit card based on its name and the balance type"""
        balance_type = "_".join(balance_type.split())
        card = self.get_credit_card(session_id, credit_card_name)
    ...
    def exchange_transaction_from_dollar_to_ruble(self, session_id: Text, amount: float):
        account = self.get_account_from_session_id(session_id)
        account_number = self.get_account_number(account)
        credit_card = (
            self.session.query(CreditCard)
        ...
        if current_dollar_balance >= amount:
            credit_card.current_ruble_balance += amount * exchange_rate
            credit_card.current_balance += amount

    def exchange_transaction_from_rub_to_dollar(self, session_id: Text, amount: float):
        account = self.get_account_from_session_id(session_id)
        account_number = self.get_account_number(account)
        credit_card = (
            self.session.query(CreditCard)
        ...
        if current_ruble_balance >= amount:
            credit_card.current_ruble_balance -= amount
            credit_card.current_balance += amount / exchange_rate